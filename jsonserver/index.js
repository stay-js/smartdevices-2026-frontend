...
function belongsToIndex({ source, destination, type }, req, data) {
  ...
  for (const item of data) {
    SetSingularItem(
      item,
      destination,
      GetDestinationArray(destination).find(
        (a) => item[`${pluralize.singular(destination)}_id`] == a.id,
      ),
    );
...
function belongsToSingle({ source, destination, type }, req, data) {
  ...
  if (req.method == 'GET' || req.method == 'POST' || req.method == 'PUT') {
    SetSingularItem(
      data,
      destination,
      GetDestinationArray(destination).filter(
        (a) => data[`${pluralize.singular(destination)}_id`] == a.id,
      ),
    );
...
function hasManyIndex({ source, destination, type }, req, data) {
  ...
  for (const item of data) {
    SetPluralItem(
      item,
      destination,
      Filter(GetDestinationArray(destination), item, `${pluralize.singular(source)}_id`, 'id'),
    );
  }
...
function hasManySingle({ source, destination, type }, req, data) {
  ...
  if (req.method == 'GET' || req.method == 'POST' || req.method == 'PUT') {
    SetPluralItem(
      data,
      destination,
      Filter(GetDestinationArray(destination), data, `${pluralize.singular(source)}_id`, 'id'),
    );
  }
  return data;
}
...
const SetSingularItem = (item, destination, value) =>
  (item[pluralize.singular(destination)] = value);
const SetPluralItem = (item, destination, value) => (item[destination] = value);

const Filter = (array, src, foreignKey, primaryKey) =>
  array.filter((dest) => parseInt(dest[foreignKey]) === parseInt(src[primaryKey]));
const Find = (array, src, foreignKey, primaryKey) =>
  array.find((dest) => parseInt(dest[foreignKey]) === parseInt(src[primaryKey]));
